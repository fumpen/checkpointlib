#ifndef CHECKPOINT2_HPP
#define CHECKPOINT2_HPP
...
private:
  ...
  MemManager2* mem;                  // pointer to 'active_checkpoint' file
  
  
public:
  
  Checkpoint2(std::uint8_t init_threadTotal, std::uint8_t init_myId, std::mutex* init_lockCheck_in,
	     std::mutex* init_lockCheck_out, std::uint8_t* init_barrier_flag_in,
	     std::uint8_t* init_barrier_flag_out, bool* init_load_first, MemManager2* init_mem){
  ...
  void check(){
	...
	*this->barrier_flag_in = 0;
	...
	this->mem->start_save_checkpoint();
	*this->barrier_flag_in = 0;
      }
    }
...
private:
  ...
  std::vector<Checkpoint2*> trackCheckInner;
  std::vector<std::thread*> trackThreads;
  
  bool load_first;                  // if true, load on first checkpoint encounter
  MemManager2* mem;                  // pointer to 'memManager' file
  
public:
  CheckpointOrganizer2(std::string init_fileNameA, std::string init_fileNameB){
    //for threading
    this->threadCount = 0;
    this->barrier_flag_in = 0;
    ...
    this->mem = new MemManager2(init_fileNameA, init_fileNameB);
  }

  ~CheckpointOrganizer2(){
    // for threading
  ...
  std::thread* startThread(void (*f)(void *, Checkpoint2*), void* init_data){
    this->threadCount += 1;
    Checkpoint2* newCheck = new Checkpoint2(this->threadTotal, this->threadCount, this->lockCheck_in,
